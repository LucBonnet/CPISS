...
import psycopg2 # Biblioteca responsavel para usar o postgree no python
...
def create_graph_for_rg(rg, visited_rg, all_conexoes):
    ...
    if conn is not None:
        try:
            ...
            if result:
                ...
                if conexoes:
                    for conexao in conexoes:
                        ...
                        if result_conexao:
                            ...
                            conexao_crimes_label = ', '.join(conexao_crimes) if conexao_crimes else 'Nenhum crime'
                            
                            # Adicionar nó para cada conexão com rótulo
                            G.add_node(conexao, label=f'{conexao}\nCrimes: {conexao_crimes_label}')
                            G.add_edge(rg, conexao)  # Adicionar aresta do RG para a conexão
                ...
                visited_rg.add(rg)
                return list(all_conexoes)  # Retornar uma lista das conexões encontradas
            else:
...
def main(start_rg):
    ...
    while to_visit:
        ...
        if current_rg not in visited_rg:
            new_conexoes = create_graph_for_rg(current_rg, visited_rg, all_conexoes)
            
            # Atualizar a lista de RGs a serem visitados com novas conexões
            to_visit.extend(conexao for conexao in new_conexoes if conexao not in visited_rg)
    
    print("Todos os grafos foram desenhados.")

# RG inicial para começar a criação dos grafos
rg_inicial = "123456789"  # Substituir pelo RG desejado
main(rg_inicial)
